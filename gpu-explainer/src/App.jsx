...
const stages = [
  ...
  },
  {
    ...
    plain: `two separate things happen between each denoising step, and most artists treat them as one dropdown. the sampler is the algorithm that decides how to remove noise at each step. the model says "i think this much noise is here." the sampler then decides what to do with that prediction. Euler takes it at face value: subtract exactly that amount, move on to the next step. DPM++ does the same thing but also remembers what the model predicted last step, and uses both predictions to estimate a smarter removal. it's like getting directions: euler follows each instruction on its own. dpm++ compares each instruction to the previous one to figure out the trend and take a better path. the scheduler is a separate thing entirely. it decides the noise levels at each step. normal spacing removes the same amount of noise per step from start to finish. Karras front-loads bigger reductions early, when the model is making the most impactful decisions (composition, structure, color), and eases into smaller adjustments at the end when it's just refining edges and texture. same number of steps, same total compute, but Karras concentrates effort where decisions matter most. these are two independent creative levers. experiment with both separately to understand what each one does to our images.`,
    detail: `mathematically, denoising traces a path from noise to image through latent space. samplers approximate this path with discrete steps. Euler: uses only the current step's noise prediction to decide the next move. simple, predictable, but needs more steps to converge. DPM++ 2M: uses both the current and previous noise predictions to estimate a curved path between steps. by comparing two predictions, it can take a smarter route and reach similar quality in fewer steps. the scheduler (normal, karras, simple) controls sigma values, which are the noise levels at each step. normal: equal-sized noise reductions throughout. Karras: larger reductions early (when composition decisions happen), smaller reductions late (when detail is being refined). the sampler and scheduler combine multiplicatively: changing either one changes the output.`,
  ...
  },
  {
    ...
    detail: `the entire time the model was working, it was sculpting a tiny 128×128 compressed version of our image. that compressed version stores information across 16 layers (some encode color, some texture, some structure, some edge detail). the VAE decoder takes those 16 layers and collapses them into the 3 color channels we actually see: red, green, and blue, at full 1024×1024 resolution. it does this in stages, doubling the resolution at each stage (128 → 256 → 512 → 1024) while progressively translating from compressed representation to real pixel colors. it's a learned decompressor: during training, it saw millions of image/latent pairs and learned how to reconstruct the full image from the compressed version.`,
